...
try
{
	$sql = "SELECT s.year, count(s.year) AS total FROM (SELECT to_char(data, 'YYYY') AS year FROM dct.tese) s GROUP BY s.year ORDER BY s.year DESC";
	...
	$sql = "SELECT *, to_char(data, 'MM-YYYY') AS fdata FROM tese WHERE date_part('year',data) = '". $year ."'";
                ...
                <div id="bloco-conteudo" class="row">
                     <div class="col-sm-10 col-md-10 col-lg-10">
                         <div class="conteudo-geral">
                            <h3 id="conteudo-titulo">Disserta&ccedil;&otilde;es de doutorado</h3>
							 	...
							 	<div class="col-md-6 filtro-buscar">
									<select class="filterform" name="year" onchange="JavaScript: document.location='?section=<?= $section ?>&year=' + this.options[this.selectedIndex].value;">
										...
										foreach ($years as $i => $t)
											echo '<option value="'. $i .'" '. ($i == $year ? 'selected="selected"' : '') .'>'. $i .': '. $t .' disserta&ccedil;&otilde;es</option>';
										?>
									</select>
								</div>
