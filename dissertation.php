...
try
{
	$sql = "SELECT s.year, count(s.year) AS total FROM (SELECT to_char(data, 'YYYY') AS year FROM dct.dissertacao) s GROUP BY s.year ORDER BY s.year DESC";
	...
	$sql = "SELECT *, to_char(data, 'MM-YYYY') AS fdata FROM dissertacao WHERE date_part('year',data) = '". $year ."'";
								...
								<?
									while ($obj = $sth->fetch (PDO::FETCH_OBJ))
									{
										$data = explode ('-', $obj->fdata);
											...
											<h2><a href="gestor/titan.php?target=openFile&fileId=<?= $obj->dissertacao ?>"><?= $obj->titulo ?></a></h2>
											<div class="informacoes"><p>Em <?= $language [$obj->idioma] ?> com <?= $obj->paginas ?> p&aacute;ginas,  <?= month ($data [0]) ?> de <?= $data [1] ?> <a href="gestor/titan.php?target=openFile&fileId=<?= $obj->dissertacao ?>">Download</a> </p></div>
